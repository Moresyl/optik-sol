...
| **结构化内容更易读** | JSON 可在折叠树和高亮代码间切换；其他正文按语言高亮并带行号、换行、有上限展开、原文查看与上下文复制 |
| **复制常驻行尾** | 控制台每行「复制」（含时间戳与调用栈），网络每行复制可重放的 cURL 命令。位置固定在同一个 x 上，不随内容长短漂移 |
...
| **键盘与触控友好** | 标签支持方向键/Home/End 导航，可展开值支持 Enter/空格，非交互值不会进入 Tab 顺序 |
