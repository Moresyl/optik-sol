...
Console, Network, Elements, Storage and Environment panels, in a single file of roughly 68 KB gzipped. Recording starts the moment the script executes, and `destroy()` restores every hook it installed. Network captures can be copied as a safe-mode HAR 1.2 archive with credentials redacted and payload bodies omitted by default.
